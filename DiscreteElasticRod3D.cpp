...
// Custom clamp function (avoid std::clamp link issues in some MinGW versions)
...
public:
    ...
    float bendK     = 5.f;
    float twistK    = 0.5f;
    float phaseLockK= 0.f;   // NEW: Phase locking stiffness
    float mass      = 0.01f;
    ...
    // Initialize a coiled rod with numNodes
    ...
    void computeForces(std::vector<Vec3> &outF, std::vector<float> &outTwistRates) {
        ...
        for (int i = 0; i < nSeg; i++) {
            ...
            outF[i]     = outF[i]     + dir * Fmag;
            outF[i + 1] = outF[i + 1] - dir * Fmag;
        }
        ...
        for (int i = 1; i < nSeg; i++) {
            ...
            float dotT = dot(t1, t2);
            dotT = clampf(dotT, -1.f, 1.f);
            ...
            outF[i]   = outF[i]   + force_i;
            outF[i+1] = outF[i+1] + force_i1;
        }
        ...
        // NEW: Phase Locking
        // Keep adjacent twist angles close to each other
        // This penalizes difference (twistAngles[i] - twistAngles[i-1])
        // distributing torque among them
        if (phaseLockK > 0.f) {
            for (int i = 1; i < nSeg; i++) {
    ...
    void step() {
        ...
        // Integrate twist with wrapping to keep angles in [-pi, pi]
        for (int i = 0; i < nSeg; i++) {
            float angVel = twistRates[i] / mass;
            twistAngles[i] += angVel * timeStep;
            // Wrap twistAngles to the interval [-π, π]
            const float PI = 3.14159f;
            const float TWO_PI = 6.28318f;
            while (twistAngles[i] > PI)  twistAngles[i] -= TWO_PI;
            while (twistAngles[i] < -PI) twistAngles[i] += TWO_PI;
        }
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 720, "3D DER - Phase Locking Example", NULL, NULL);
    if(!window){
